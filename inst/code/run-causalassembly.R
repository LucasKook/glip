...
ca <- import("causalAssembly.models_dag", convert = TRUE)

# Parse command line arguments
args <- commandArgs(trailingOnly = TRUE)
mode <- darg(args[1], "admg")
n <- as.numeric(darg(args[2], 10000))
...
reg <- darg(args[9], "lrm")
nsim <- as.numeric(darg(args[10], 10))
test <- "gcm"
save <- TRUE

### Folders
outdir <- file.path("inst", "results", "causalAssembly", Sys.Date(), paste0("d", d_max, "ms", ms))

### Read data
assembly_line_data <- ca$ProductionLineGraph$get_data()
data <- odata <- assembly_line_data[sample.int(nrow(assembly_line_data), n), ]
assembly_line <- ca$ProductionLineGraph$get_ground_truth()
gt <- as.matrix(assembly_line$between_adjacency)
V <- oV <- colnames(data)
d <- od <- NCOL(data)
...
out <- lapply(seq_len(nsim), \(iter) {
  ...
  if (!use_oracle_tests) {
    tests <- tests_ms <- learn_graph(
      data = data, max_size = use_ms, mode = mode, test_args = targs,
      return_tests_only = TRUE, test = test
    )
  }
  if (ms < d - 2) {
