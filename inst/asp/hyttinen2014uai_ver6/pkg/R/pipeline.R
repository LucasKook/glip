pipeline <- function(n = 4, topology = "random", # model properties
  ...
  if (is.null(model)) {
    M <<- randomGraph(n = n, restrict = restrict, topology = topology, pedge = 1 / (n - 1))
  ...
  true_graph <- M$G
  dimnames(true_graph) <- list(letters[1:n], letters[1:n])

  true_graph_bid <- M$Ge
  dimnames(true_graph_bid) <- list(letters[1:n], letters[1:n])

  if (all(restrict != "sufficient")) {
    true_graph <- list(M1 = true_graph, M2 = true_graph_bid)
  }
  md <<- mode

  ws <- eh <- NULL
  if (test != "oracle") {
    suff <- list(C = cor(data.frame(D[[1]]$data)), n = N)
    pcres <- pcalg::pc(suff, pcalg::gaussCItest,
      labels = letters[1:n], alpha = p
    )
    pcout <- as(pcres@graph, "matrix")
    fcires <- pcalg::fciPlus(suff, pcalg::gaussCItest,
      selectionBias = FALSE,
      verbose = FALSE, labels = letters[1:n], alpha = p
    )
    fciout <- as(fcires@amat, "matrix")
    ws <- if (md %in% c("dag", "dg")) .ess_to_dag(pcout) else .pag_to_admg(fciout)
    eh <- if (md %in% c("dag", "dg")) 1 * (pcout != 0) else 1 * (fciout != 0)
  }

  if (test == "oracle") {
    tests <- .compute_oracle_tests(true_graph, max_size = schedule, mode = md)
    lG <<- .get_opt(md)(tests, d = n, max_size = schedule, gurobi_args = GARGS,
      weight_type = WTYPE)
  } else {
    lG <<- learn_graph(data.frame(D[[1]]$data),
      # test_args = list(reg_YonZ = "lrm", reg_XonZ = "lrm"),
      test = "gaussCItest", comets = FALSE,
      gurobi_args = GARGS, weight_type = WTYPE, trafo = \(x) 1 * (x <= p),
      max_size = schedule, mode = md, warmstart = ws, edgehints = eh
    )$graph
  }

  obj <- sapply(lG$optim$pool, \(x) x$objval)

  glip <- as.difftime(lG$optim$runtime, units = "secs")
  glopt <- min(lG$optim$objval)
  asp <- as.difftime(L$solving_time, units = "secs")
  aspopt <- aspobj

  # asp_graph <- L$G
  # dimnames(asp_graph) <- list(letters[1:n], letters[1:n])
  #
  # LG <- list(M1 = asp_graph, M2 = L$Ge)
  # dimnames(LG$M2) <- dimnames(asp_graph)
  # print(.compute_graphical_representation(LG, n - 2, md))
  # print(true_graph)
  # print(.compute_graphical_representation(true_graph, n - 2, md))
  # print(.compute_graphical_representation(lG$graph, n - 2, md))
  # print(L)
  return(data.frame(glip = glip, asp = asp, glip_opt = glopt, asp_opt = aspopt, glip_ws = max(obj)))
  ...
  objective_M <- objective(M, global_indeps,
    asp = (solver == "clingo"),
